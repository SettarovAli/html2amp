...
const minifyCssVariables = (cssString, ignoreCssVariablesList = []) => {
  const variableMapping = {}
  const variablePattern = /--[\w-]+/g
  let minifiedNameCounter = 0

  return cssString.replace(variablePattern, (variableName) => {
    if (ignoreCssVariablesList.includes(variableName)) {
      return variableName
    }
...
const minifyCss = async ($, cssString, ignoreCssVariablesList) => {
  const purgedCss = await purgeCss($, cssString)
  return minifyCssVariables(purgedCss, ignoreCssVariablesList)
}
...
const css = async ($, options) => {
  ...
  const minifiedCss = await minifyCss($, cssString, options.ignoreCssVariablesList)
